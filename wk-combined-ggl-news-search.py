...
def main():
    ...
    additional_keywords = ["estafar", "denunciar", "demandar", "robar"]

    results = []

    for agent in education_agents:
        for keyword in additional_keywords:
            combined_term = f'"{agent}" {keyword}'
            try:
                articles = search_google_news(combined_term, max_results=3)
                count = len(articles)
                print(f'Search term: "{combined_term}", Articles found: {count}')
                for title, url in articles:
                    results.append({"Search Term": combined_term, "News Title": title, "News Link": url})
            except Exception as e:
                print(f'Error occurred for search term: "{combined_term}"')
                print(f"Error message: {str(e)}")

    df = pd.DataFrame(results)
    df.to_excel("google_news_spanish_combined_top3.xlsx", index=False)
    print("Results saved to google_news_spanish_combined_top3.xlsx")
