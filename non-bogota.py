...
def run_report():
    ...
    # - Exclude visitors from Bogotá by applying a NOT filter on the "city" dimension.
    request = RunReportRequest(
        ...
        dimension_filter=FilterExpression(
            not_expression=FilterExpression(
                filter=Filter(
                    field_name="city",
                    string_filter=Filter.StringFilter(value="Bogota")
                )
            )
        )
    )
...
def process_response(response):
    ...
    for month in sorted(monthly_users.keys()):
        ...
        month_date = datetime.strptime(f"2024-{month}-01", "%Y-%m-%d")
        data.append({
            "Month": month_date.strftime("%Y-%m"),
            "Non-Bogota Users": monthly_users[month]
        })
    return data
...
def export_to_excel(data):
    ...
    output_file = "non_bogota_users_2024.xlsx"
